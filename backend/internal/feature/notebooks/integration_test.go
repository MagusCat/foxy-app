...
//
// Run with:  DATABASE_URL=postgresql://... go test ./internal/feature/notebooks/ -run Foreign
...
func TestForeignUserGets404(t *testing.T) {
	...
	z, err := repo.Create(ctx, alice, CreateNotebookRequest{Name: "Zona de Alice"}, nil)
	if err != nil {
		t.Fatalf("Alice could not create her notebook: %v", err)
	}

	// Bob must not be able to see or delete it.
	if _, err := repo.Get(ctx, bob, z.ID); !errors.Is(err, apperr.ErrNotFound) {
		t.Fatalf("Bob got Alice's notebook (expected NOT_FOUND): %v", err)
	}
	if err := repo.Delete(ctx, bob, z.ID); !errors.Is(err, apperr.ErrNotFound) {
		t.Fatalf("Bob could delete Alice's notebook (expected NOT_FOUND): %v", err)
	}

	// Alice can.
	if _, err := repo.Get(ctx, alice, z.ID); err != nil {
		t.Fatalf("Alice could not see her own notebook: %v", err)
	}
}
